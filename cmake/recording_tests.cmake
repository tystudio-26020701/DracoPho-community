qt_add_executable(dracoPho-recording-capture-backend-test
...
target_include_directories(dracoPho-recording-capture-backend-test PRIVATE src)
target_link_libraries(dracoPho-recording-capture-backend-test
    PRIVATE
        Qt6::Core
        Qt6::Test
)
add_test(NAME recording-capture-backend COMMAND dracoPho-recording-capture-backend-test)

qt_add_executable(dracoPho-recording-bgra-buffer-pool-test
...
target_include_directories(dracoPho-recording-bgra-buffer-pool-test PRIVATE src)
target_link_libraries(dracoPho-recording-bgra-buffer-pool-test
    PRIVATE
        Qt6::Core
        Qt6::Test
)
add_test(NAME recording-bgra-buffer-pool COMMAND dracoPho-recording-bgra-buffer-pool-test)

if(MARK_SHOT_LINUX AND PipeWire_FOUND)
    qt_add_executable(dracoPho-pipewire-buffer-data-types-test
    ...
    target_include_directories(dracoPho-pipewire-buffer-data-types-test PRIVATE src)
    target_link_libraries(dracoPho-pipewire-buffer-data-types-test
        PRIVATE
            Qt6::Core
            Qt6::Test
            PkgConfig::PipeWire
    )
    add_test(NAME pipewire-buffer-data-types COMMAND dracoPho-pipewire-buffer-data-types-test)
endif()

qt_add_executable(dracoPho-recording-dialog-config-test
...
target_include_directories(dracoPho-recording-dialog-config-test PRIVATE src)
target_link_libraries(dracoPho-recording-dialog-config-test
    PRIVATE
        Qt6::Core
        Qt6::Gui
        Qt6::Test
)
add_test(NAME recording-dialog-config COMMAND dracoPho-recording-dialog-config-test)

qt_add_executable(dracoPho-recording-config-dialog-test
...
target_include_directories(dracoPho-recording-config-dialog-test PRIVATE src)
target_link_libraries(dracoPho-recording-config-dialog-test
    PRIVATE
        Qt6::Core
        Qt6::Gui
        Qt6::Widgets
...
if(PulseAudioRecording_FOUND)
    target_compile_definitions(dracoPho-recording-config-dialog-test PRIVATE HAVE_PULSE_RECORDING)
    target_link_libraries(dracoPho-recording-config-dialog-test PRIVATE PkgConfig::PulseAudioRecording)
endif()
# Windows 需要 WASAPI 读取器依赖与 windows_integration 的系统库，
# 与主目标保持一致，避免 Windows 测试链接失败。
if(WIN32)
    target_link_libraries(dracoPho-recording-config-dialog-test PRIVATE dwmapi ksuser ole32 uuid)
endif()
add_test(NAME recording-config-dialog COMMAND dracoPho-recording-config-dialog-test)

if(FFmpegLibav_FOUND)
    qt_add_executable(dracoPho-libav-recording-process-test
    ...
    target_include_directories(dracoPho-libav-recording-process-test PRIVATE src)
    target_compile_definitions(dracoPho-libav-recording-process-test PRIVATE HAVE_LIBAV_RECORDING)
    if(PulseAudioRecording_FOUND)
        target_compile_definitions(dracoPho-libav-recording-process-test PRIVATE HAVE_PULSE_RECORDING)
    endif()
    target_link_libraries(dracoPho-libav-recording-process-test
        PRIVATE
            Qt6::Core
            Qt6::Gui
            Qt6::Test
    ...
    if(PulseAudioRecording_FOUND)
        target_link_libraries(dracoPho-libav-recording-process-test PRIVATE PkgConfig::PulseAudioRecording)
    endif()
    if(WIN32)
        target_link_libraries(dracoPho-libav-recording-process-test PRIVATE ksuser ole32 uuid)
    endif()
    add_test(NAME libav-recording-process COMMAND dracoPho-libav-recording-process-test)
endif()
