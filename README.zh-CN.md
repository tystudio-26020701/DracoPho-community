...
- **富文本标注**：
  - 支持超大字号的无级调节，可通过鼠标滚轮平滑缩放。
  - 引入物理宽度缓冲区设计，避免文本在极高缩放比例下由于渲染抖动产生意外折行。
  - **八个控制点**与矩形一致，仅调整文本框几何；字号恒定，文字随框自动重排换行宽度。
...
- **无头 CLI / 脚本化** — 太殷龙摄 提供完整无头链路：`--capture-to` 区域 / 显示器截图、`--list-windows` + `--window` 窗口 / 组件截图、`--record-region` / `--record-display` 无人值守录制、`--record-wait-json` 等待并输出结构化状态；全程无窗口、无弹窗、无焦点抢占，可指定 **PID / 进程名**并截取被遮挡 / 最小化窗口。ShareX（仅 Windows）、Flameshot、ksnip、Spectacle 有较强 CLI；Snipaste 的 CLI 为付费版功能；PixPin 仅有 JS 脚本动作引擎。
...
| **T** | 文本 (Text) | 输入并编排富文本，支持选区局部格式化；手柄仅调整文本框几何，字号恒定、文字随框重排。 |
