...
이 매뉴얼은 DracoPho의 일상적인 사용법을 다룹니다. 특히 **창 / 구성 요소 호버 선택** 기능(마우스를 움직이면 커서 아래에 있는 창을 자동으로 추적·강조 표시하고, 클릭하면 선택하는 기능), 주석(annotation) 작업 흐름, 헤드리스(headless) 캡처, 그리고 설정에 초점을 맞춥니다.
...
런처는 자신의 `bin/` 디렉터리를 `PATH` 앞에 추가합니다. 이는 창 감지 헬퍼 스크립트(`dracoPho-window-detection-*`)와 OCR / 업로드 헬퍼에 필요합니다.

### 1.3 시작 동작 (플로팅 볼 / 트레이 / 설정 창 / 직접 캡처)

DracoPho을 시작할 때(예: 바탕화면 아이콘 클릭) 어떤 일이 벌어지는지 설정할 수
있습니다. **설정 → 일반 → 시작 동작**을 열고 원하는 옵션을 조합하세요:

| 옵션 | 시작 시 동작 |
| :--- | :--- |
| **트레이 아이콘**(기본값) | DracoPho이 시스템 트레이에서 백그라운드로 실행됩니다. 트레이 아이콘 왼쪽 클릭으로 빠른 캡처, 오른쪽 클릭으로 메뉴가 열립니다. |
| **플로팅 볼**(기본값) | 항상 위에 표시되는 작은 공(기본값 화면 오른쪽 아래)이 나타납니다. 한 번 클릭하면 빠른 메뉴, 두 번 클릭하면 바로 캡처, 드래그하면 이동(위치가 기억됩니다)하며 캡처 중에는 자동으로 숨겨집니다. |
| **직접 캡처** | 시작 즉시 캡처 모드로 들어갑니다. |
| **설정 창** | DracoPho 시작 시 설정 창을 엽니다. |

여러 옵션을 조합할 수 있습니다. **직접 캡처는 기본적으로 꺼져 있습니다**. 선택하지
않는 한 시작 시 캡처 오버레이가 자동으로 열리지 않습니다. 모든 옵션을 해제하면
트레이로 폴백하여 항상 진입점이 남습니다.

플로팅 볼과 트레이 메뉴에는 동일한 빠른 작업이 있습니다: **캡처**,
**전체 화면 캡처**, **녹화 시작**, **설정**, **종료**. 볼 메뉴에는
**플로팅 볼 숨기기**도 있습니다.
...
DracoPho은 영역을 선택하기 전에 현재 데스크톱의 창들을 감지할 수 있습니다. 선택 오버레이가 열려 있는 동안 **마우스를 움직이면 커서 아래의 창이 청록색(teal) 테두리로 강조 표시됩니다.** **드래그 없이 단순 왼쪽 클릭을 하면 해당 창 전체가 캡처 영역으로 선택**됩니다. 이후 바로 주석을 달거나, 복사, 핀, 저장할 수 있습니다.
...
1. 캡처를 트리거합니다(`dracoPho` 또는 데스크톱 단축키).
...
이 기능은 기본적으로 활성화되어 있습니다(`windowDetection.enabled = true`). **설정 → 고급 → 창 감지 활성화(Window Detection Enabled)** 에서 전환하거나 `~/.config/dracoPho/config.json`을 편집하세요:
...
- `command`: 감지 스크립트. GNOME / KDE / Hyprland / niri Wayland에서는 세션과 일치하는 번들 `dracoPho-window-detection-*` 스크립트가 자동으로 선택됩니다. X11과 Windows에서는 플랫폼이 프로세스 내에서 열거되므로 `command`를 비워 두어도 됩니다. **사용자가 제공한 사용자 지정 명령(예: 절대 경로)은 항상 존중됩니다.**
...
| 진단 내용 확인 | `dracoPho --debug --debug-log /tmp/dracoPho.log` 실행 — `window-detection` 줄을 확인하세요 |
...
| `T` | 텍스트(Text) | 서식 있는 텍스트, 선택 영역 부분 서식 지정 지원. 휠로 글자 크기 조절. 8개 핸들은 사각형과 동일하게 상자 기하만 조절하며 글자 크기는 일정하게 유지되고 텍스트는 상자 너비에 맞춰 재배열됨. 글꼴 패널에서 정확한 pt 크기, 글꼴 패밀리, 굵게 / 기울임꼴 설정 |
...
- 텍스트를 더블 클릭하면 그 자리에서 편집 (텍스트(Text) 도구로 기존 텍스트 상자를 클릭해도 그 자리에서 선택해 편집할 수 있음).

속성 패널(오른쪽)에서는 선택한 주석 유형에 따른 속성을 편집합니다: 도형은 색상 / 두께 / 스타일, 텍스트는 글꼴 패밀리 / 크기 / 굵게 / 기울임꼴에 더해 문자 색상, 문자 강조, 상자 배경색, 불투명도 슬라이더를 제공합니다. 편집 중 부분 선택이 있으면 문자 색상과 문자 강조는 해당 선택 영역에만 적용되고, 없으면 상자 전체 텍스트에 적용됩니다. 상자 배경색과 불투명도는 항상 상자 전체에 적용됩니다. `선택` 도구에서 선택 상자를 드래그하면 여러 주석을 선택할 수 있으며, 그룹을 함께 이동, 크기 조절, 회전, 삭제할 수 있습니다.
...
3. GNOME Wayland에는 DracoPho Scroll Helper 확장(§ 2.1)이 필요합니다.
...
있습니다. `dracoPho --record-region 0,0,640,480 --record-output ~/Videos/clip.mp4
...
DracoPho은 **특정 창 — 또는 창 내부의 구성 요소(하위 영역) — 을 UI를 열지 않고 캡처**할 수 있습니다. 스크립트, 빌드 파이프라인 또는 에이전트에서 사용할 수 있습니다. 프로세스는 이미지가 기록되거나 반환되는 즉시 종료되며, 창을 만들지 않고, 대화 상자를 띄우지 않으며, 포커스를 빼앗지 않으므로 도구가 데스크톱을 캡처하는 동안에도 사용자는 계속 작업할 수 있습니다.
...
| `stage` | 임시 스테이징 디렉터리(`$TMPDIR/dracoPho-staging`)에 PNG 파일 기록. "나중에 보관" 워크플로에 좋습니다. |
...
트레이 모드(`dracoPho --tray`)는 영역 캡처용 `Ctrl+Alt+S`를 등록하고 캡처 / 녹화 / 설정 / 종료 메뉴 항목을 제공합니다. 데스크톱 단축키:

- **GNOME**: 설정 → 키보드 → 단축키 → 사용자 지정 단축키 → `dracoPho`에 바인딩.
- **KDE**: `dracoPho`에 바인딩된 사용자 지정 단축키(정확한 KDE 캡처를 위한 KWin ScreenShot2 권한 포함, README 참조).
- **Hyprland**: `bind = SUPER SHIFT, S, exec, dracoPho` 및 `bind = , Print, exec, dracoPho`.
...
- 설정 파일: `~/.config/dracoPho/config.json`(Linux), 첫 실행 시 생성됩니다.
...
9. **트레이 + 단축키** — `dracoPho --tray`, `Ctrl+Alt+S` 누르기.
...
번들에 포함된 [이슈 제출 가이드](../.doc/submit-issue-via-gh.md)를 사용하여 `gh issue create`로 문제를 보고하세요. `dracoPho --debug --debug-log /tmp/dracoPho.log`로 캡처한 디버그 로그를 첨부하세요.
