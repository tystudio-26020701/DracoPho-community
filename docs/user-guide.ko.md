...
Mark Shot을 시작할 때(예: 바탕화면 아이콘 클릭) 어떤 일이 벌어지는지 설정할 수
있습니다. **설정 → 일반 → 시작 동작**을 열고 원하는 옵션을 조합하세요:

| 옵션 | 시작 시 동작 |
| :--- | :--- |
| **트레이 아이콘**(기본값) | Mark Shot이 시스템 트레이에서 백그라운드로 실행됩니다. 트레이 아이콘 왼쪽 클릭으로 빠른 캡처, 오른쪽 클릭으로 메뉴가 열립니다. |
