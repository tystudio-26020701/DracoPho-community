...
太殷龙摄 在 Linux 上从 `~/.config/dracoPho/config.json` 读取应用配置，在其他平台上使用 Qt 应用配置目录。贴图窗口同样使用该文件中的 OCR 与翻译配置。默认 OCR helper 会优先使用 `rapidocr`，也可以回退到 `tesseract`。翻译 helper 使用 OpenAI 兼容的 `/chat/completions` 接口。
...
| `debug.logPath` | 字符串 | 系统临时目录 `dracoPho-scroll.log` | 调试日志输出路径。命令行参数 `--debug-log` 会覆盖此项；未设置配置或命令行路径时，`MARK_SHOT_DEBUG_LOG` 仍然有效。 |
...
| `save.pathTemplate` | 字符串 | `"{pictures}/dracoPho/dracoPho-{datetime}.png"` | 保存截图文件的路径模板（包括保存动作和另存为的初始文件名）。父级目录在保存时若不存在会自动创建。别名包括：`save.path`、`save.location`、最外层的 `savePathTemplate` 以及 `save.directory`（目录模板）。 |
| `save.directoryTemplate` | 字符串 | `""` | 仅指定保存目录模板。指定后，文件名会自动采用默认的 `dracoPho-{datetime}.png`。别名包括：`save.directory`、`save.dir`、`save.folder`。 |
| `recording.storage.videoDirectory` | 字符串 | `"{pictures}/dracoPho/videos"` | MP4 录制文件默认输出目录。别名包括 `recording.storage.videos`、`recording.storage.videoDir` 和 `recording.output.videoDirectory`。 |
| `recording.storage.gifDirectory` | 字符串 | `"{pictures}/dracoPho/gifs"` | GIF 与动画 WebP 录制文件默认输出目录。别名包括 `recording.storage.gifs`、`recording.storage.gifDir` 和 `recording.output.gifDirectory`。 |
...
| `windows.tray.enabled` | 布尔值 | Windows 为 `true`，其他平台为 `false` | 自动启动托盘模式。键名出于兼容性保留。可以用 `dracoPho --tray` 在不修改配置时启动托盘模式，也可以用 `dracoPho --capture` 在自动启动托盘时强制执行单次截图。 |
...
| `codeScan.command` | 字符串 | `""` | 自定义二维码/条形码扫码命令。支持 `{image}`、`{imagePath}` 和 `{imageUrl}` 占位符；如果没有占位符，太殷龙摄 会把临时 PNG 路径追加到命令末尾。命令必须输出与 `dracoPho-code-scan` 相同结构的 JSON。别名：`codeScanner.command`、`barcodeScanner.command`、`barcode.command`。 |
| `codeScan.timeoutMs` | 数值 | `15000` | 扫码命令超时时间。环境变量 `MARK_SHOT_CODE_SCAN_TIMEOUT_MS` 可以覆盖该值。 |
| `upload.command` | 字符串 | `""` | 自定义图床上传命令。支持 `{image}`、`{imagePath}` 和 `{imageUrl}` 占位符；如果没有占位符，太殷龙摄 会把临时 PNG 路径追加到命令末尾。命令必须输出 JSON `{"url":"...","deleteUrl":"...","errors":[]}` 或纯文本 URL（以 `http://`/`https://` 开头）。留空时使用内置 `dracoPho-upload` 脚本，通过 `upload.env` 配置图床参数。别名：`imageUpload.command`、`uploader.command`、`imageHost.command`。 |
| `upload.timeoutMs` | 数值 | `60000` | 上传命令超时时间。环境变量 `MARK_SHOT_UPLOAD_TIMEOUT_MS` 可以覆盖该值。 |
| `upload.env` | 对象 | `{}` | 传递给上传命令的环境变量。会合并到系统环境变量之上。用于配置内置 `dracoPho-upload` 脚本的图床参数（端点、字段、API Key、认证方案、URL 提取路径等）。别名：`environment`、`envVars`、`variables`。 |
...
| `floatingBall.alwaysOnTop` | 布尔 | `true` | 悬浮球是否始终保持在普通窗口上方。悬浮球每次显示时都会重新强化置顶状态（Windows 原生 `HWND_TOPMOST`，GNOME Wayland 经随附 Shell 扩展），因此截图会话隐藏/恢复后仍能回到置顶。设为 `false` 可关闭置顶行为。 |
...
*注：相对路径会被解析至默认的 `Pictures/dracoPho` 目录下，若路径模板未以 `.png` 结尾，程序会自动追加。*
...
状态文件独立保存在 `~/.config/dracoPho/annotation-state.json`（Linux 平台；其他平台使用 Qt 应用配置目录），与 `config.json` 完全分离。该文件只用于保存临时的工具默认值，可随时删除以重置编辑器到内置默认值。
...
- **GNOME Shell**：`dracoPho-window-detection-gnome`
...
2. 为该脚本赋予可执行权限：
   ...
   ```
3. 在您的 `~/.config/dracoPho/config.json` 配置文件中，在 `windowDetection.command` 字段中指定该脚本的名称（如果在 `$PATH` 中）或其绝对路径：
   ```json
   "windowDetection": {
     "command": "dracoPho-window-detection-niri"
   }
   ...
   ```json
   "windowDetection": {
     "command": "dracoPho-window-detection-kde",
     "timeoutMs": 2000
   }
   ```
...
| 环境变量名称 | 数据类型 | 描述 |
| :--- | :--- | :--- |
| `MARK_SHOT_CONFIG` | 字符串 | 配置文件路径（通常为 `~/.config/dracoPho/config.json`） |
...
手动安装时，必须同时安装 `dracoPho`、`dracoPho-ocr`、`dracoPho-code-scan`、`dracoPho-translate` 和 `dracoPho-upload`。否则 OCR、扫码、翻译或图床上传功能无法调用后端脚本。
