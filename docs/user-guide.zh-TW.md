...
本手冊涵蓋 太殷龍攝 的日常操作，重點在於**視窗／元件懸停選取**功能（移動滑鼠時會自動追蹤並反白游標下方的視窗；按一下即可選取它）、註記工作流程、無頭擷取以及設定。
...
啟動 太殷龍攝（例如點擊桌面圖示）後會發生什麼，是可配置的。打開
**設定 → 通用 → 啟動行為**，任意勾選組合：

| 選項 | 啟動時行為 |
| :--- | :--- |
| **托盤圖示**（預設） | 太殷龍攝 在系統托盤保持背景執行。左鍵單擊托盤圖示快速截圖，右鍵打開選單。 |
| **懸浮球**（預設） | 螢幕右下角出現一個可拖動的置頂小球。單擊彈出快捷選單，雙擊直接截圖，拖動可改變位置（位置會被記住）。截圖進行中自動隱藏。 |
| **直接截圖** | 啟動後立即進入截圖模式。 |
| **設定視窗** | 啟動 太殷龍攝 時打開設定視窗。 |

多個選項可組合使用。**直接截圖預設關閉**；除非勾選它，否則啟動 太殷龍攝
不會自動彈出截圖覆蓋層。若所有選項都被取消，太殷龍攝 會回退為托盤執行，
保證始終有入口。

懸浮球與托盤選單提供相同的快捷操作：**截圖**、**全螢幕截圖**、**開始錄製**、
**設定**、**退出**。懸浮球選單額外提供**隱藏懸浮球**，可將其隱藏到下次啟動。

---

## 2. 視窗／元件懸停選取

太殷龍攝 可以在您選取區域之前偵測目前桌面的視窗。當選取疊加層開啟時，**移動滑鼠會以青色框反白游標下方的視窗**。**直接左鍵按一下（不拖曳）會選取整個視窗**作為擷取區域；接著您可以直接進行註記、複製、釘選或儲存。
...
| `E` | 橢圓 | 橢圓／圓形 |
| `A` | 箭頭 | 經典箭頭（羽狀、KDE、雙向） |
| `T` | 文字 | 富文字，支援局部選取範圍格式化；滾輪調整字號；八個控點與矩形一致，僅調整文字框幾何、字號恆定，文字隨框自動重排；字型面板提供精確 pt 輸入、字型家族、粗體／斜體 |
...
- 連點兩下文字即可就地編輯（以「文字」工具點按既有文字框同樣可就地選取編輯）。

屬性面板（右側）會依所選註記類型顯示對應屬性：形狀為顏色、寬度、樣式；文字為字型家族／大小／粗體／斜體、文字顏色、文字高亮、文字框底色與不透明度滑桿。編輯時若有局部選取範圍，文字顏色與文字高亮僅作用於該範圍，無選取範圍時則作用於整框文字；文字框底色與不透明度永遠作用於整框。在「選取」工具下拖曳一個選取框可以選取多個註記；之後整個群組可以一起移動、調整大小、旋轉與刪除。
...
3. GNOME Wayland 需要 太殷龍攝 Scroll Helper 擴充功能（第 2.1 節）。
...
太殷龍攝 可以**不開啟任何使用者介面**，就從腳本、建置管線或代理程式擷取**特定視窗——或視窗內的元件（子區域）**。程序只要寫入或回傳影像後就會結束，而且絕不建立視窗、絕不跳出對話方塊、絕不搶佔焦點，因此工具擷取桌面時使用者可以繼續工作。
...
{"count":2,"platform":"wayland","source":"compositor-script","windows":[
  {"index":0,"id":"0x3c00007","title":"太殷龍攝 - VSCodium","class":"codium","instance":"codium","x":1920,"y":0,"width":1680,"height":1050,"zOrder":1},
