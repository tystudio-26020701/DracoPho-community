...
啟動 Mark Shot（例如點擊桌面圖示）後會發生什麼，是可配置的。打開
**設定 → 通用 → 啟動行為**，任意勾選組合：

| 選項 | 啟動時行為 |
| :--- | :--- |
| **托盤圖示**（預設） | Mark Shot 在系統托盤保持背景執行。左鍵單擊托盤圖示快速截圖，右鍵打開選單。 |
| **懸浮球**（預設） | 螢幕右下角出現一個可拖動的置頂小球。單擊彈出快捷選單，雙擊直接截圖，拖動可改變位置（位置會被記住）。截圖進行中自動隱藏。 |
| **直接截圖** | 啟動後立即進入截圖模式。 |
| **設定視窗** | 啟動 Mark Shot 時打開設定視窗。 |

多個選項可組合使用。**直接截圖預設關閉**；除非勾選它，否則啟動 Mark Shot
不會自動彈出截圖覆蓋層。若所有選項都被取消，Mark Shot 會回退為托盤執行，
保證始終有入口。
