...
| `floatingBall.edgeDockEnabled` | Boolean | `true` | Whether the floating ball docks to a screen edge and hides half of itself when dropped near an edge. Effective on all platforms: on X11/Windows/macOS the window is moved to the edge and the ball slides half off-screen; on native Wayland the ball content is offset toward the edge (window position is compositor-controlled), so drag the ball against the edge for the best result. |
