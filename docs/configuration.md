...
| `history.enabled` | Boolean | `true` | Whether copied, saved and pinned screenshots are recorded into the capture history (tray/floating-ball "Capture History..." entry). |
| `history.maxEntries` | Integer | `50` | Maximum number of capture history entries kept on disk; `0` keeps all captures. |
...
| `floatingBall.alwaysOnTop` | Boolean | `true` | Whether the floating ball stays above normal windows. The ball re-asserts the topmost state every time it is shown (Windows native `HWND_TOPMOST`, GNOME Wayland via the bundled Shell extension), so it recovers its topmost position after capture sessions hide and re-show it. Set to `false` to disable the always-on-top behavior. |
| `floatingBall.edgeDockEnabled` | Boolean | `true` | Whether the floating ball docks to a screen edge and hides half of itself when dropped near an edge. Only effective on X11/Windows/macOS, where the window is moved to the edge and the ball slides half off-screen. On native Wayland the compositor controls window positions and the client cannot read reliable absolute coordinates, so edge docking is unavailable (protocol limitation): the ball stays free-floating (idle fade still works on every platform). |
| `floatingBall.hiddenExtentPx` | Number | `22` | How many pixels of the floating ball slide off-screen while docked (the ball's radius, so roughly half the ball stays visible as an interactive sliver). Clamped below the ball diameter. |
| `floatingBall.idleFadeSeconds` | Number | `3` | Delay in seconds before the floating ball fades to `idleOpacity` after the mouse leaves it. `0` disables idle fading. |
