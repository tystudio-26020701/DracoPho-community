...
| `E` | 椭圆 | 椭圆 / 正圆 |
| `A` | 箭头 | 经典箭头（实心、KDE、双向） |
| `T` | 文字 | 富文本，支持选区局部格式化；滚轮调节字号；八个手柄与矩形一致，仅调整文本框几何、字号恒定，文字随框自动重排；字体面板支持精确 pt 输入、字体族、粗体 / 斜体 |
...
- 双击文字：就地编辑（`文字` 工具点击既有文本框同样可就地选中编辑）。

右侧属性面板随所选标注类型显示对应属性：形状为颜色、宽度、样式；文字为字体 /
字号 / 粗体 / 斜体、文字颜色、文字高亮、文本框底色与不透明度滑条。编辑态存在
局部选区时，文字颜色与文字高亮仅作用于选区，无选区时作用于整框文字；文本框
底色与不透明度永远作用于整框。`选择` 工具下拖出选框可多选，多选后可整体移动、缩放、旋转、删除。
