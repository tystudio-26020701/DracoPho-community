...
| `--capture-destination <mode>` | وجهة الصور: `inline` (base64 في مخرجات JSON) أو `file` أو `stage` أو `clipboard`. تنطبق على التقاط النوافذ وعلى التقاط الشاشة — عدا `clipboard` (رمز الخروج 2). |
