...
- **سلوك بدء تشغيل قابل للتكوين**: لم يعد تشغيل Mark Shot يفتح طبقة التقاط الشاشة افتراضيًا. في الإعدادات → عام → **سلوك بدء التشغيل** يمكنك الجمع بين الأنماط الأربعة: **أيقونة شريط النظام** و**الكرة العائمة** و**نافذة الإعدادات** و**التقاط مباشر**. التثبيتات الجديدة تعتمد افتراضيًا أيقونة شريط النظام + الكرة العائمة؛ والالتقاط المباشر اختياري.
...
الإصدار المجتمعي من Mark Shot هو أداة متكاملة **مفتوحة المصدر (MIT)، متعددة المنصات (أصلي على لينكس X11/Wayland + Windows)، وقابلة للعمل دون اتصال تماماً** تجمع التقاط الشاشة والتحرير والتثبيت وOCR والترجمة وتسجيل الشاشة. جُمع الجدولان أدناه من الوثائق والمواقع الرسمية لكل منتج (حتى **أغسطس 2026**)، مع توثيق القدرات بصدق: **✅ دعم مدمج**؛ **⭕ دعم جزئي أو يعتمد على أدوات/خدمات خارجية**؛ **❌ غير مدعوم**. تُحسب القدرات كلها على أساس "جاهزة للاستخدام فوراً" ولا ترتبط بعضويات مدفوعة أو خدمات سحابية أو فروع تجريبية.

### نظرة عامة على القدرات الأساسية

| القدرة | الإصدار المجتمعي Mark Shot | Flameshot | ShareX | PixPin | Snipaste | Spectacle | Snagit | CleanShot X | Shottr |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| لقطة منطقة | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| لقطة ملء الشاشة | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| لقطة نافذة | ⭕ | ❌ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
...
**جودة وشمولية اللقطة** (تحدد "هل يمكنك التقاط ما تريد")

- **لقطة منطقة / ملء الشاشة / شاشة محددة** — يدعم Mark Shot كل هذه الأنواع، ويتعامل بشكل صحيح مع الإحداثيات السالبة والقياس المستقل لكل شاشة (HiDPI) في إعدادات الشاشات المتعددة؛ وقد غطت المنافسات السائدة هذه القدرة الأساسية أيضاً.
- **لقطة نافذة** — يدعم الوضع بدون واجهة في Mark Shot (`--window`) بالكامل اختيار النافذة بالمعرّف أو العنوان أو الفئة أو PID أو اسم العملية؛ وعلى X11 يقرأ المحتوى الحقيقي مباشرة من المخزن المؤقت للنافذة الذي يحتفظ به مدير التركيب حتى لو كانت محجوبة أو مصغّرة (دون إظهارها أو انتزاع التركيز). أما الوضع التفاعلي فيستخدم طبقة تحديد بملء الشاشة ولا يوفر وضع "النافذة النشطة" بضغطة واحدة (لذلك رُمز بـ ⭕).
- **لقطة التمرير الطويلة** — تعمل في Mark Shot أصلياً على niri وWayland في GNOME (يتطلب الامتداد الرسمي لـ GNOME)، وهي ميزة تجريبية على KDE/X11. توفرها ShareX على Windows وCleanShot X / Shottr على macOS بشكل مدمج؛ بينما لا تدعمها Flameshot وSpectacle وSnipaste في معسكر المصدر المفتوح.
- **لقطة مؤجلة / مجدولة** — غير متوفرة حالياً في Mark Shot (❌)؛ تدعمها معظم الأدوات السائدة وتُعد بنوداً قابلة للإضافة لاحقاً.

**التحرير والتعرف والخصوصية** ("هل يمكنك استخدام اللقطة بكفاءة بعد الالتقاط")

- **مجموعة أدوات التحرير** — يضم Mark Shot أكثر من 12 أداة مدمجة: القلم وقلم التمييز والخط والمستطيل والقطع الناقص والسهم والنص وأرقام الخطوات والفسيفساء والعدسة المكبرة بإطارين ومؤشر الليزر وقطّارة الألوان والمسطرة؛ في المستوى نفسه مع ksnip وSpectacle وShareX.
- **تثبيت اللقطة Pin** — يدعم Mark Shot لقطات مثبتة بلا حدود وفي المقدمة مع التكبير والتدوير والتقاط كلمات OCR والترجمة الموضعية عبر LLM؛ وتضمها أيضاً Snipaste وShareX وCleanShot X وShottr.
- **التعرف على النصوص OCR** — يضم Mark Shot RapidOCR مدمجاً (نموذج PP-OCR، دون اتصال) مع تراجع إلى Tesseract، جاهزاً للاستخدام فوراً؛ لا تتوفر في Snipaste وFlameshot وSnagit (أو تتطلب إضافات)، وتعتمد Spectacle على Tesseract خارجي.
- **الترجمة** — يضم Mark Shot ترجمة لقطات الشاشة عبر LLM بواجهة متوافقة مع OpenAI (تعمل دون اتصال/بخدمة ذاتية)؛ **لا يضم أي من المنافسين ترجمة لقطات شاشة عامة مدمجة** (ترجمة PixPin ميزة مدفوعة، وترجمة WeChat/QQ مربوطة بعميلَيها). هذه من أبرز مزايا التمايز حالياً.
- **التعرف على رموز QR / الباركود** — يتيح Mark Shot مسحاً مدمجاً للرموز (QR / رموز أحادية البعد / PDF417 وغيرها)، بالوصول المباشر عبر مفتاح `Q` في مرحلة اللقطة؛ تدعمه ShareX وShottr بينما لا تمتلكه معظم الأدوات.
- **إخفاء تلقائي بالذكاء الاصطناعي بضغطة واحدة** — لم يضم Mark Shot بعد خاصية الإخفاء التلقائي التي أضافها WeChat للقطات عام 2026 (❌)، وهي اتجاه قابل للمتابعة؛ تتوفر حالياً في Mark Shot الفسيفساء/الزجاج المصنفر كتحرير يدوي.

**المشاركة والتعاون والأتمتة** ("هل يمكنك توزيع اللقطة/الاحتفاظ بها بسهولة بعد الالتقاط")

- **رفع إلى استضافة الصور** — يضم Mark Shot مدمجاً ImgURL / sm.ms / imgbb / litterbox / أوامر مخصصة، مع الرفع بضغطة واحدة ونسخ الرابط عبر `Ctrl+U`؛ تمتلك ShareX أكبر عدد من وجهات الرفع، ويدعم Flameshot Imgur فقط، بينما لا يوفر Snipaste / PixPin أي استضافة.
- **CLI بدون واجهة / سكربتات** — يوفر Mark Shot مساراً كاملاً بدون واجهة: `--capture-to` للقطات المنطقة/الشاشات، و`--list-windows` + `--window` للقطات النوافذ/المكونات، و`--record-region/--record-display` للتسجيل دون مراقبة، و`--record-wait-json` للانتظار وإخراج حالة منظمة؛ طوال المسار بلا نوافذ ولا نوافذ منبثقة ولا انتزاع للتركيز، مع **إمكانية تحديد PID / اسم العملية** وتصوير النوافذ المحجوبة/المصغّرة. يعيد خادم MCP في الإصدار التجاري استخدام المسار نفسه. من بين المنافسين يملك ShareX (على Windows فقط) وFlameshot فقط CLI قوياً؛ ولا يقدم PixPin إلا محرك إجراءات سكربتات JS.
- **الكرة العائمة للوصول السريع** — يضم Mark Shot كرة عائمة قابلة للسحب والالتصاق بحواف الشاشة وتتلاشى تلقائياً عند الخمول؛ لا يوفرها سوى PixPin (المدفوع).

**المنصات والترخيص والبيئة**

- **Wayland أصلي على لينكس** — يدعم Mark Shot أصلياً PipeWire portal وgrim وlayer-shell وKDE KWin ScreenShot2 وامتدادات GNOME؛ وفي معسكر المصدر المفتوح يصل Spectacle (KDE) فقط إلى المستوى الأصلي نفسه، بينما دعم Flameshot لـ Wayland تجريبي. لا توجد إصدارات لينكس لـ PixPin / Snipaste (Pro).
- **مفتوح المصدر / مجاني** — الإصدار المجتمعي من Mark Shot مفتوح المصدر برخصة MIT ومجاني تماماً بلا إعلانات ولا حسابات ولا حاجة للاتصال بالشبكة؛ Flameshot وShareX وSpectacle وShottr مجانية/مفتوحة المصدر أيضاً، بينما PixPin / Snipaste تجاريان مغلقا المصدر (عضوية/مدفوعة)، وSnagit / CleanShot X برامج تجارية مدفوعة.
- **النظام البيئي للإضافات** — يوفر Mark Shot نظام إضافات Qt وسوق إضافات على GitHub (موفرو OCR / ترجمة / مسح الرموز قابلون للتوسعة)؛ تمتلك ShareX إضافات إجراءات/رفع، ويملك ksnip إضافة OCR، بينما لا تمتلك Flameshot / Spectacle أي نظام إضافات.

> ملاحظة: استندت المقارنة أعلاه إلى المواقع والوثائق الرسمية لكل منتج (2026-08)؛ تتغير نطاقات الوظائف مع تطور الإصدارات، فارجع إلى أحدث وثائق كل منتج. لم تُدرج بعض الأدوات (FastStone Capture وNimbus ولقطات Sogou وغيرها) في الجدول الشامل، وتُعرف قدراتها وفق معيار البحث أعلاه.
