...
- **تعليق نص منسّق**:
  - يدعم ضبطاً لانهائياً لأحجام خطوط كبيرة جداً، مع تكبير سلس عبر عجلة الفأرة.
  - يقدّم تصميم مخزن مؤقت بالعرض الفيزيائي لتجنّب التفاف النص المفاجئ الناتج عن اهتزاز العرض عند نسب التكبير العالية جداً.
  - **المقابض الثمانية** تغيّر شكل إطار النص تمامًا كما في المستطيل؛ يبقى حجم الخط ثابتًا ويعاد لفّ النص حسب العرض الجديد.
...
- `--capture-screen --capture-destination inline` يعيد PNG بصيغة Base64 داخل JSON (حقل `data`) دون كتابة أي ملف؛ ويجتمع مع `--region`/`--display`/`--all-outputs`.
- `--capture-screen --capture-destination stage` يكتب في دليل الترحيل المؤقت (`/tmp/dracoPho-staging/`)؛ وجمعه مع `--capture-to` مرفوض كتكرار.
- `--capture-destination` مجرد مُعدِّل لا يُطلق الالتقاط بمفرده؛ وأي مُعدِّل بدون مُطلق (`--capture-to`/`--capture-screen`/`--window`) يُنهي بالرمز 2.
...
| `--capture-screen` | التقاط شاشة بدون مسار؛ يتطلب صراحةً `--capture-destination inline` أو `stage`. |
...
| `--list-displays` | يطبع الشاشات المتاحة بصيغة JSON ثم يخرج. |
| `--doctor` | يطبع تشخيصًا ذاتيًا JSON للبيئة (الإصدار، Qt/نظام التشغيل/الجلسة، الشاشات، إعدادات الوضع بلا واجهة، كشف النوافذ) ثم يخرج. |
| `--delay <seconds>` | ينتظر العدد المحدد من الثواني قبل الالتقاط: في الوضع التفاعلي عدّاد تنازلي ملء الشاشة (Esc للإلغاء)؛ وفي الوضع بلا واجهة انتظار صامتة بلا نوافذ (0–3600، والقيم غير الصالحة تخرج بالرمز 2). |
| `--capture-destination <mode>` | مُعدِّل فقط لا يُطلق الالتقاط: `inline`/`file` (مع `--capture-to`)/`stage` (مع `--capture-screen`)/`clipboard` (للنوافذ فقط). منفردًا أو بمزاج خاطئ: رمز 2. |
...
| **T** | النص (Text) | لإدخال نص منسّق وتنظيمه مع تنسيق التحديد؛ المقابض تغيّر شكل إطار النص فقط ويبقى حجم الخط ثابتًا. |
