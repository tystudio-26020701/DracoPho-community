...
- **Захват области / полноэкранный / многомониторный** — встроен во все 15 инструментов из списка; Mark Shot дополнительно корректно обрабатывает дисплеи с отрицательными координатами и независимое масштабирование HiDPI для каждого дисплея, без «призраков» при склейке многомониторных снимков.
- **Захват окна** — интерактивный режим Mark Shot использует полноэкранный слой выделения (без отдельного однокнопочного режима «активное окно», поэтому ⭕), а безголовый режим (`--window`) точно выбирает окна по id, заголовку, классу, **PID или имени процесса**; у Flameshot режима окна нет вовсе, у всех остальных инструментов он есть.
- **Содержимое перекрытых / свёрнутых окон** — в X11 Mark Shot считывает собственный композитируемый буфер окна через XComposite, захватывая реальное содержимое, даже если окно полностью перекрыто или свёрнуто (без вызова на передний план и без перехвата фокуса); среди аналогичных инструментов на это способен только `--stack` утилиты `scrot`. Протокол Wayland не позволяет ни одному инструменту считывать содержимое свёрнутых окон.
- **Прокручиваемый / длинный скриншот** — Mark Shot работает с ним нативно в niri и GNOME Wayland (официальное расширение), в KDE/X11 это экспериментальная функция. ShareX, PixPin, PicPick, Snagit, CleanShot X, Shottr, Xnip и iShot имеют встроенную поддержку; Greenshot — только для устаревших сценариев IE; Snipaste, Flameshot, ksnip, Spectacle и Windows Snipping Tool — нет.
- **Снимок по таймеру / с задержкой** — в Mark Shot пока недоступен (❌); поддерживается большинством популярных инструментов, кроме PixPin, Snipaste и Xnip, — естественный кандидат на добавление в будущем.

**II. Аннотации и интеллектуальные функции** (можно ли эффективно использовать снимок после его создания)

- **Набор инструментов аннотаций** — в Mark Shot встроено более 12 инструментов: кисть, маркер, линия, прямоугольник, эллипс, стрелка, текст, нумерация шагов, мозаика, двухрамочная лупа, лазерная указка, пипетка и линейка; во всех 15 инструментах есть встроенные аннотации разной глубины.
- **Закрепление на экране (Pin)** — Mark Shot предлагает безрамные закреплённые поверх всех окон стикеры с масштабированием, поворотом, выделением слов через OCR и переводом LLM прямо на стикере; Snipaste, PixPin, ShareX, ksnip, CleanShot X, Shottr, Xnip и iShot тоже поддерживают его; у Flameshot, Spectacle, Greenshot, PicPick, Snipping Tool и Snagit — нет.
- **Распознавание текста (OCR)** — в Mark Shot встроены RapidOCR (офлайн-модели PP-OCR) с откатом на Tesseract, работает из коробки; Windows Snipping Tool (локальные текстовые действия), ShareX, Snagit, CleanShot X, Shottr, Xnip и iShot имеют его; Snipaste скрывает его за платной подпиской, ksnip — за плагином, Spectacle — за внешней установкой Tesseract; у Flameshot, Greenshot и PicPick его нет.
- **Перевод** — в Mark Shot встроен перевод скриншотов через LLM по OpenAI-совместимому интерфейсу (поддерживается офлайн / собственная служба). Встроенный перевод скриншотов есть только у iShot, у PixPin он скрыт за платной подпиской, а больше ни один инструмент его не предлагает. Это остаётся одним из самых сильных отличий.
- **QR / распознавание штрихкодов** — Mark Shot сканирует QR, одномерные коды и PDF417 клавишей `Q` на этапе создания снимка; ShareX и PixPin поддерживают его; Snipaste (платно) и CleanShot X / Shottr / iShot распознают только QR — собственными движками OCR.
- **Пипетка (выбор цвета)** — в Mark Shot встроена экранная пипетка с палитрой истории; большинство инструментов имеют её, кроме Spectacle и Snagit; Windows Snipping Tool — только на ИИ-ПК с Copilot+.
- **Автоматическое замыливание в один клик (AI)** — автоматическая функция замыливания WeChat 2026 года пока не встроена (❌); мозаика / размытие в Mark Shot — ручные. У Snagit (AI Smart Redact) и функции замыливания текста в Snipping Tool есть сопоставимые реализации; автоматическое замыливание — возможное направление развития.

**III. Вывод и автоматизация** (можно ли опубликовать / сохранить результат)

- **Запись экрана** — Mark Shot записывает MP4 / GIF / анимированный WebP с тихой записью области/дисплея без участия пользователя (без окон, диалогов и перехвата фокуса); ShareX, PixPin, Spectacle (Plasma 6), PicPick, Snipping Tool, Snagit, CleanShot X и iShot имеют её; у Snipaste, Flameshot, ksnip, Greenshot, Shottr и Xnip — нет.
- **Анимированный GIF / WebP** — Mark Shot нативно записывает **анимированный WebP** (меньший размер файлов, поддержка альфа-канала); ни один конкурент не создаёт анимированный WebP нативно. PixPin и iShot записывают GIF, ShareX записывает GIF (WebP — только статическое сохранение), Snagit и CleanShot X экспортируют GIF; у остальных анимированного вывода нет.
- **Загрузка на хостинг изображений** — в Mark Shot встроены ImgURL / sm.ms / imgbb / litterbox / загрузка пользовательской командой (`Ctrl+U`); у ShareX наибольшее число целей; у Greenshot, PicPick, Snagit и CleanShot X есть встроенная облачная загрузка; Flameshot — только Imgur, ksnip — Imgur/FTP/скрипты, Shottr — S3 после активации; у Snipaste, PixPin, Snipping Tool, Xnip и iShot её нет.
- **Безголовый CLI / скриптинг** — Mark Shot предоставляет полный безголовый конвейер: `--capture-to` для снимков области/дисплея, `--list-windows` + `--window` для снимков окна/компонента, `--record-region` / `--record-display` для записи без участия пользователя и `--record-wait-json` для структурированного вывода статуса — всё без окон, диалогов и перехвата фокуса, с **нацеливанием по PID/имени процесса и захватом перекрытых/свёрнутых окон**. MCP-сервер коммерческой версии использует тот же конвейер. У ShareX (только Windows), Flameshot, ksnip и Spectacle полноценные CLI; CLI Snipaste — платная функция; PixPin предлагает только движок JS-действий.
- **Плавающий шар быстрого запуска** — в Mark Shot есть перетаскиваемый шар, который прилипает к краям экрана (X11/Windows/macOS; на Wayland он свободно плавает из-за ограничений протокола) и становится полупрозрачным в простое; аналогичную функцию предлагает только PixPin.
- **История снимков** — у Mark Shot пока нет отдельной панели истории; у ShareX, Snipaste, Flameshot, Snagit и CleanShot X она есть, ksnip / PicPick / Shottr / iShot имеют облегчённые альтернативы (вкладки / галерея / закреплённые хранилища), у остальных её нет.

**IV. Платформы и экосистема**

- **Нативный Linux Wayland** — Mark Shot нативно поддерживает PipeWire portal, grim, layer-shell, KDE KWin ScreenShot2 и расширения GNOME; среди инструментов с открытым исходным кодом лишь Spectacle (KDE) достигает такого же уровня, а Flameshot / ksnip экспериментальны или зависят от portal.
- **Кроссплатформенность** — Snipaste, Flameshot, ksnip и Snagit охватывают все три основные настольные ОС (Windows + macOS + Linux); Mark Shot охватывает Linux + Windows (macOS запланирована); ShareX, PicPick, Snipping Tool, Spectacle, CleanShot X, Shottr, Xnip и iShot — одноплатформенные.
- **Механизм плагинов / расширений** — Mark Shot предоставляет систему плагинов на Qt и маркет плагинов на GitHub (расширяемые провайдеры OCR / перевода / сканирования кодов); у ksnip и Greenshot есть API плагинов; ShareX, Spectacle, Snagit, CleanShot X и Shottr заменяют их пользовательскими действиями / интеграциями.
- **Открытый исходный код / бесплатно** — Mark Shot CE распространяется по лицензии MIT, полностью бесплатен, без рекламы, без аккаунтов и без обязательного подключения к сети; ShareX, Flameshot, ksnip, Spectacle и Greenshot (Windows) тоже открыты и бесплатны; Shottr и Snipping Tool бесплатны; PixPin / Snipaste / PicPick / Xnip / iShot — закрытые условно-бесплатные (freemium); Snagit / CleanShot X — платное коммерческое ПО.
