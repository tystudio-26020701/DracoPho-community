...
- **Форматированный текст**:
  - Бесступенчатая регулировка очень крупных размеров шрифта — плавное масштабирование колесом мыши.
  ...
  - **Все восемь контрольных точек** меняют геометрию рамки точно как у прямоугольника; размер шрифта остаётся неизменным, а текст переносится по новой ширине.
...
- **Кроссплатформенность** — Snipaste, Flameshot, ksnip и Snagit охватывают все три основные настольные ОС (Windows + macOS + Linux); DracoPho охватывает Linux + Windows + macOS (macOS 15 проверена); ShareX, PicPick, Snipping Tool, Spectacle, CleanShot X, Shottr, Xnip и iShot — одноплатформенные.
...
- `--capture-screen --capture-destination inline` возвращает PNG в Base64 внутри JSON (поле `data`), не записывая файлов; сочетается с `--region`/`--display`/`--all-outputs`.
- `--capture-screen --capture-destination stage` пишет во временный каталог стейджинга (`/tmp/dracoPho-staging/`); вместе с `--capture-to` отклоняется как избыточное.
- `--capture-destination` — чистый модификатор и сам по себе захват не запускает; любой модификатор без триггера (`--capture-to`/`--capture-screen`/`--window`) завершается кодом 2.
...
| `--capture-screen` | Захват экрана без пути; требует явного `--capture-destination inline` или `stage`. |
...
| `--list-displays` | Выводит доступные дисплеи в формате JSON и завершает работу. |
| `--doctor` | Выводит JSON-диагностику окружения (версия, Qt/ОС/сессия, дисплеи, настройки headless, обнаружение окон) и завершает работу. |
| `--delay <seconds>` | Ждёт указанное количество секунд перед захватом: в интерактивном режиме — полноэкранный обратный отсчёт (Esc для отмены); в headless — тихое ожидание без окон (0–3600, недопустимые значения завершают работу с кодом 2). |
| `--capture-destination <mode>` | Чистый модификатор, не триггер: `inline`, `file` (с `--capture-to`), `stage` (с `--capture-screen`) или `clipboard` (только окна). Отдельно или в неверной паре: код 2. |
...
| **T** | Текст (Text) | Ввод и оформление форматированного текста с форматированием выделения; ручки меняют только геометрию рамки, размер шрифта остаётся неизменным. |
