...
Mark Shot Community Edition — это **открытая (MIT), кроссплатформенная (нативная поддержка Linux X11/Wayland + Windows) и полностью автономная (офлайн)** интегрированная утилита для скриншотов, аннотаций, закрепления на экране, OCR, перевода и записи экрана. Таблицы ниже составлены по официальной документации и сайтам продуктов (по состоянию на **август 2026 года**) и честно отражают наличие возможностей: **✅ встроенная поддержка**; **⭕ частичная поддержка или зависимость от внешних инструментов/сервисов**; **❌ не поддерживается**. Возможности учитываются по принципу «из коробки» — без привязки к платным подпискам, облачным сервисам или экспериментальным веткам.

### Сводная таблица возможностей

| Возможность | Mark Shot Community Edition | Flameshot | ShareX | PixPin | Snipaste | Spectacle | Snagit | CleanShot X | Shottr |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| Захват области | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
...
| Набор инструментов аннотаций | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| Нумерация шагов / аннотации лупой | ✅ | ❌ | ✅ | ✅ | ❌ | ✅ | ✅ | ✅ | ✅ |
...
**Качество и охват снимков** (определяет, «можно ли захватить то, что нужно»)

- **Захват области / полноэкранный / дисплея** — Mark Shot поддерживает всё это, корректно обрабатывая отрицательные координаты и независимое масштабирование (HiDPI) в многомониторных конфигурациях; все основные конкуренты также покрывают эту базовую возможность.
- **Захват окна** — в безголовом режиме (`--window`) Mark Shot полностью поддерживает выбор окна по id / заголовку / классу / PID / имени процесса; в X11 он считывает реальное содержимое из сохранённого композитором буфера окна, даже если окно перекрыто или свёрнуто (без вызова на передний план и без перехвата фокуса). Интерактивный режим представляет собой полноэкранный слой выделения и не имеет отдельного однокнопочного режима «активное окно» (поэтому отмечен как ⭕).
- **Прокручиваемый длинный скриншот** — Mark Shot работает с ним нативно в niri и GNOME Wayland (для GNOME требуется официальное расширение), в KDE/X11 это экспериментальная функция. ShareX и Snagit (Windows), а также CleanShot X / Shottr (macOS) также имеют встроенную поддержку; среди инструментов с открытым исходным кодом Flameshot, Spectacle и Snipaste её не имеют.
- **Снимок по таймеру / с задержкой** — в Mark Shot пока недоступен (❌); большинство популярных инструментов поддерживают его, это возможное дополнение в будущем.

**Аннотации, распознавание и конфиденциальность** («можно ли эффективно использовать снимок после его создания»)

- **Набор инструментов аннотаций** — в Mark Shot встроено более 12 инструментов: кисть, маркер, линия, прямоугольник, эллипс, стрелка, текст, нумерация шагов, мозаика, двухрамочная лупа, лазерная указка, пипетка и линейка; на одном уровне с ksnip, Spectacle и ShareX.
- **Закрепление (Pin)** — Mark Shot поддерживает безрамное закреплённое поверх всех окон изображение, масштабирование, поворот, выделение слов через OCR и перевод LLM прямо на закреплённом изображении; Snipaste, ShareX, CleanShot X и Shottr также имеют встроенную поддержку.
- **Распознавание текста (OCR)** — в Mark Shot встроены RapidOCR (модели PP-OCR, офлайн) с откатом на Tesseract, работает из коробки; у Snipaste, Flameshot и Snagit его нет (или нужны плагины), а Spectacle зависит от внешнего Tesseract.
- **Перевод** — в Mark Shot встроен перевод скриншотов через LLM по OpenAI-совместимому интерфейсу (возможен офлайн / собственная служба); **ни один из конкурентов не имеет встроенного универсального перевода скриншотов** (у PixPin это платная функция, а перевод в WeChat/QQ привязан к их клиентам). Это одна из самых уникальных отличительных возможностей на данный момент.
- **QR / распознавание штрихкодов** — в Mark Shot встроено сканирование кодов (QR / одномерные коды / PDF417 и т. д.), доступное на этапе создания скриншота клавишей `Q`; ShareX и Shottr поддерживают его, большинство инструментов — нет.
- **Автоматическое замыливание в один клик (AI)** — появившуюся в 2026 году автоматическую функцию замыливания в скриншотах WeChat Mark Shot пока не имеет (❌), это возможное направление развития; существующие мозаика/матовое стекло в Mark Shot — это ручные аннотации.

**Обмен, совместная работа и автоматизация** («можно ли легко распространить или сохранить снимок после его создания»)

- **Загрузка на хостинг изображений** — в Mark Shot встроены ImgURL / sm.ms / imgbb / litterbox / пользовательские команды, загрузка одним нажатием `Ctrl+U` с копированием ссылки; у ShareX наибольший выбор целей загрузки, Flameshot поддерживает только Imgur, у Snipaste / PixPin хостинга изображений нет.
- **Безголовый CLI / скриптинг** — Mark Shot предоставляет полный безголовый конвейер: `--capture-to` для снимков области/дисплея, `--list-windows` + `--window` для снимков окна/компонента, `--record-region/--record-display` для записи без участия пользователя и `--record-wait-json` для ожидания и вывода структурированного статуса; всё это без окон, диалогов и перехвата фокуса, причём **можно указать PID / имя процесса** и захватывать перекрытые/свёрнутые окна. MCP-сервер коммерческой версии использует тот же конвейер. Среди конкурентов лишь ShareX (только Windows) и Flameshot имеют сопоставимый CLI; у PixPin есть только движок JS-скриптовых действий.
- **Плавающий шар быстрого запуска** — в Mark Shot есть перетаскиваемый шар, который прилипает к краям экрана и становится полупрозрачным в простое; аналогичную функцию предлагает только PixPin (по платной подписке).

**Платформы, лицензирование и экосистема**

- **Нативный Linux Wayland** — Mark Shot нативно поддерживает PipeWire portal, grim, layer-shell, KDE KWin ScreenShot2 и расширения GNOME; среди инструментов с открытым исходным кодом лишь Spectacle (KDE) достигает такого же уровня нативности, а поддержка Wayland у Flameshot экспериментальная. У PixPin / Snipaste (Pro) нет версии для Linux.
- **Открытый исходный код / бесплатно** — Mark Shot Community Edition — это открытый код по лицензии MIT, полностью бесплатный, без рекламы, без аккаунтов и без обязательного подключения к сети; Flameshot, ShareX, Spectacle и Shottr также бесплатны или открыты, PixPin / Snipaste — закрытые коммерческие продукты (подписка/лицензия), а Snagit / CleanShot X — платное коммерческое ПО.
- **Экосистема плагинов** — Mark Shot предоставляет систему плагинов на Qt и маркет плагинов на GitHub (расширяемые провайдеры OCR / перевода / сканирования кодов); у ShareX есть плагины действий/загрузки, у ksnip — плагин OCR, а у Flameshot / Spectacle системы плагинов нет.

> Примечание: сравнение составлено по официальным сайтам и документации продуктов (2026-08); набор возможностей меняется с выходом новых версий, поэтому ориентируйтесь на актуальную документацию каждого продукта. Некоторые инструменты (FastStone Capture, Nimbus, Sogou Capture и др.) не включены в сводную таблицу; их возможности соответствуют описанной выше методологии оценки.
