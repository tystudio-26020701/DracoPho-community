...
- **GIF・MP4・アニメーション WebP の録画**: 起動時の録画ショートカットまたはトレイメニューから、指定したディスプレイやカスタム領域を GIF、MP4、またはアニメーション WebP として録画できます。録画はクラッシュセーフです: MP4 は一時的な MKV に書き出され、終了時にリマックスされるため、中断された録画も復旧可能です。録画中はトレイと凍結フレームにステータスが表示され、`S`、オーバーレイのボタン、トレイメニュー、または `--stop-recording` で停止できます。開始時と保存時にはデスクトップ通知が送信されます。Wayland では録画は PipeWire portal バックエンドを優先して使用し、portal のキャプチャが利用できない場合は wlroots screencopy またはポーリングキャプチャにフォールバックします。
...
- **フローティングボール**: 小さな常時最前面の丸いボール（既定は画面右下）で、スクリーンショット・全画面スクリーンショット・録画・設定へのクイックアクセスを提供します。シングルクリックでメニュー、ダブルクリックで直接スクリーンショット、ドラッグで移動（位置は記憶されます）。スクリーンショット中は自動的に非表示になります。
  画面端の近くにドラッグすると自動的に吸着し、**端側の半分が画面外に隠れます**（ホバーすると表示され、離すと再び非表示になります。全プラットフォームで有効）。数秒間アイドル状態が続くと半透明にフェードアウトし、ホバーするとすぐに不透明度が戻ります。
- **デスクトップショートカット**:
...
Mark Shot コミュニティ版は、**オープンソース（MIT）、クロスプラットフォーム（Linux X11/Wayland ネイティブ + Windows）、完全オフライン**のスクリーンショット・注釈・ピン・OCR・翻訳・録画を一体化したツールです。下表は各製品の公式ドキュメントと公式サイト（**2026 年 8 月**時点）をもとに、機能の有無をそのまま表記したものです：**✅ 内蔵対応**；**⭕ 部分対応または外部ツール・サービスに依存**；**❌ 非対応**。機能はすべて「初期状態ですぐ使える（アウトオブザボックス）」基準で集計しており、有料会員プラン・クラウドサービス・実験的なブランチには連動していません。

### コア機能の概要

| 機能 | Mark Shot コミュニティ版 | Flameshot | ShareX | PixPin | Snipaste | Spectacle | Snagit | CleanShot X | Shottr |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 領域スクリーンショット | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| 全画面スクリーンショット | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| ウィンドウスクリーンショット | ⭕ | ❌ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
...
**スクリーンショットの品質と対応範囲**（「必要なものを撮れるかどうか」）

- **領域 / 全画面 / ディスプレイスクリーンショット** — Mark Shot はすべてに対応しており、マルチディスプレイ環境では負の座標とディスプレイごとの独立スケーリング（HiDPI）も正しく処理します。主要な競合製品もこの基本機能をすべて備えています。
- **ウィンドウスクリーンショット** — Mark Shot はヘッドレスモード（`--window`）で id / タイトル / クラス / PID / プロセス名によるウィンドウ選択を完全にサポートしています。X11 では、ウィンドウが遮蔽されている場合や最小化されている場合でも、コンポジタが保持するウィンドウバッファから実際の内容を直接読み取れます（前面へのポップアップやフォーカスの奪取はありません）。対話モードは全画面の選択オーバーレイであり、「アクティブウィンドウ」のワンクリックモードは別途用意されていないため ⭕ としています。
- **スクロール長尺スクリーンショット** — Mark Shot は niri と GNOME Wayland でネイティブに利用できます（GNOME では公式拡張機能が必要）。KDE/X11 では実験的な機能です。Windows の ShareX・Snagit と macOS の CleanShot X / Shottr も内蔵しています。オープンソース勢の Flameshot・Spectacle・Snipaste はいずれも非対応です。
- **タイマー / 遅延スクリーンショット** — Mark Shot は現時点では未提供です（❌）。多くの主要ツールが対応しており、今後追加できる項目です。

**注釈・認識・プライバシー**（「撮影後、効率よく使えるか」）

- **注釈ツールセット** — Mark Shot は 12 種類以上のツールを内蔵しています：ペン、蛍光ペン、直線、矩形、楕円、矢印、テキスト、ステップ番号、モザイク、2 フレーム拡大鏡、レーザーポインター、カラーピッカー、ルーラー。ksnip・Spectacle・ShareX と同等のレベルです。
- **デスクトップピン** — Mark Shot は枠なしの常時最前面ピン、拡大縮小、回転、OCR 文字選択、LLM によるその場での翻訳をサポートします。Snipaste・ShareX・CleanShot X・Shottr も内蔵しています。
- **OCR 文字認識** — Mark Shot は RapidOCR（PP-OCR モデル、オフライン）+ Tesseract フォールバックを内蔵し、初期状態ですぐ使えます。Snipaste・Flameshot・Snagit は非対応（またはプラグインが必要）、Spectacle は外部の Tesseract に依存します。
- **翻訳** — Mark Shot は OpenAI 互換 API の LLM によるスクリーンショット翻訳を内蔵しています（オフライン / 自前サーバーに対応）。**競合製品には汎用のスクリーンショット翻訳を内蔵したものはありません**（PixPin は有料会員機能、WeChat / QQ の翻訳はそれぞれのクライアントに紐づいています）。これは現在最も独自性の高い差別化機能の 1 つです。
- **QR コード / バーコード認識** — Mark Shot はコードスキャンを内蔵しており（QR / 一次元コード / PDF417 など）、スクリーンショット中に `Q` キーで直接呼び出せます。ShareX・Shottr は対応していますが、多くのツールは非対応です。
- **ワンクリック AI ぼかし** — WeChat のスクリーンショットが 2026 年に追加した自動ぼかし機能は、Mark Shot にはまだ内蔵されていません（❌）。今後の対応候補です。Mark Shot の既存のモザイク / 毛ガラスぼかしは手動注釈です。

**共有・連携・自動化**（「撮影後、そのまま共有・蓄積できるか」）

- **画像ホストへのアップロード** — Mark Shot は ImgURL / sm.ms / imgbb / litterbox / カスタムコマンドを内蔵し、`Ctrl+U` でワンクリックアップロードしてリンクをコピーできます。ShareX はアップロード先が最も多く、Flameshot は Imgur のみ、Snipaste / PixPin には画像ホスト機能がありません。
- **ヘッドレス CLI / スクリプト化** — Mark Shot は完全なヘッドレスパイプラインを提供します：`--capture-to` による領域・ディスプレイのスクリーンショット、`--list-windows` + `--window` によるウィンドウ・コンポーネントのスクリーンショット、`--record-region` / `--record-display` による無人録画、`--record-wait-json` による待機と構造化ステータス出力。すべてウィンドウ・ダイアログなし・フォーカス奪取なしで動作し、**PID / プロセス名を指定**して遮蔽・最小化されたウィンドウも撮影できます。商用版の MCP サーバーは同じパイプラインを再利用しています。競合では ShareX（Windows のみ）と Flameshot だけが本格的な CLI を備えており、PixPin は JS スクリプトのアクションエンジンのみです。
- **フローティングボールのショートカット** — Mark Shot はドラッグ可能で画面端に吸着し、アイドル時には自動的に半透明になるフローティングボールを内蔵しています。同様の機能を持つのは PixPin（有料会員）のみです。

**プラットフォーム・ライセンス・エコシステム**

- **Linux Wayland ネイティブ** — Mark Shot は PipeWire portal、grim、layer-shell、KDE KWin ScreenShot2、GNOME 拡張機能をネイティブにサポートしています。オープンソース勢では Spectacle（KDE）だけが同等のネイティブレベルに達しており、Flameshot の Wayland 対応は実験的です。PixPin / Snipaste（Pro）には Linux 版がありません。
- **オープンソース / 無料** — Mark Shot コミュニティ版は MIT ライセンスのオープンソースで、完全無料・広告なし・アカウント不要・ネットワーク接続不要です。Flameshot・ShareX・Spectacle・Shottr も無料のオープンソース / 無料であり、PixPin / Snipaste はクローズドソースの商用製品（会員 / ライセンス）、Snagit / CleanShot X は有料の商用ソフトウェアです。
- **プラグインエコシステム** — Mark Shot は Qt プラグインシステムと GitHub プラグインマーケットを提供しています（OCR / 翻訳 / コードスキャンの provider を拡張可能）。ShareX にはアクション / アップロードプラグイン、ksnip には OCR プラグインがありますが、Flameshot / Spectacle にはプラグインシステムがありません。

> 補足：上記の比較は各製品の公式サイトと公式ドキュメント（2026-08）をもとに作成しています。機能の基準はバージョンの進化に伴って変わる可能性があるため、各製品の最新ドキュメントを参照してください。一部のツール（FastStone Capture、Nimbus、Sogou スクリーンショットなど）は総覧表に含まれていませんが、その機能については上記の調査基準を参照してください。
...
| `--record-region <x,y,w,h>` | 実行中のインスタンスを介して画面領域を録画します。ジオメトリは x,y,幅,高さ です。 |
| `--record-display <id>` | ID でディスプレイを録画します（`--list-displays` の出力を参照: `DP-1` などの生の画面名、`output:DP-1` キー、または `all`）。 |
| `--record-output <path>` | 録画の出力ファイルパス（録画フラグ使用時に必須）。 |
| `--record-duration <seconds>` | 録画時間（秒）。0 は `--stop-recording` まで録画します。 |
| `--record-fps <n>` | 録画のフレームレート（デフォルト 15）。 |
| `--record-format <mp4\|gif\|webp>` | 録画フォーマット: mp4（デフォルト）、gif、または webp。 |
| `--record-audio` | 録画にシステムオーディオを含めます。 |
| `--record-wait-json` | 録画が終了するまで待機し、最終ステータスを JSON で出力します。 |
